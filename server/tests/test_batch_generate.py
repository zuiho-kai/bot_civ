...
- WakeupService.scheduled_trigger() 返回多个 agent
- delayed_send 错开广播
"""
...
# ===========================================================================
# scheduled_trigger 返回多个 agent 测试
# ===========================================================================

@pytest.mark.asyncio
async def test_scheduled_trigger_returns_list(db):
    """scheduled_trigger 返回 list[int]"""
    from app.models import Agent, Message
    from app.services.wakeup_service import WakeupService

    # 创建 agents
    db.add(Agent(id=0, name="Human", persona="人类", model="none"))
    db.add(Agent(id=1, name="Alice", persona="友好的助手", model="m1"))
    db.add(Agent(id=2, name="Bob", persona="幽默的伙伴", model="m1"))
    db.add(Message(agent_id=0, sender_type="human", content="大家好", message_type="chat"))
    await db.commit()

    svc = WakeupService()

    # mock 模型返回两个名字
    with patch("app.services.wakeup_service.call_wakeup_model", new_callable=AsyncMock, return_value="Alice,Bob"):
        result = await svc.scheduled_trigger({0, 1, 2}, db)
...
@pytest.mark.asyncio
async def test_scheduled_trigger_returns_empty_when_none(db):
    """模型返回 NONE 时返回空列表"""
    from app.models import Agent, Message
    from app.services.wakeup_service import WakeupService

    db.add(Agent(id=0, name="Human", persona="人类", model="none"))
    db.add(Agent(id=1, name="Alice", persona="友好", model="m1"))
    db.add(Message(agent_id=0, sender_type="human", content="你好", message_type="chat"))
    await db.commit()

    svc = WakeupService()

    with patch("app.services.wakeup_service.call_wakeup_model", new_callable=AsyncMock, return_value="NONE"):
        result = await svc.scheduled_trigger({0}, db)

    assert result == []


@pytest.mark.asyncio
async def test_scheduled_trigger_no_human_online(db):
    """人类不在线时返回空列表"""
    ...
    svc = WakeupService()
    result = await svc.scheduled_trigger({1}, db)  # 只有 agent 在线，没有 human(0)
    assert result == []
...
def _make_candidate(id, name):
    """创建候选 Agent mock，正确设置 name 属性"""
    m = MagicMock()
    m.id = id
    m.name = name
    return m
...
def test_resolve_names_comma_separated():
    """逗号分隔的名称正确解析"""
    from app.services.wakeup_service import WakeupService

    svc = WakeupService()
    candidates = [_make_candidate(1, "Alice"), _make_candidate(2, "Bob"), _make_candidate(3, "Charlie")]

    result = svc._resolve_names("Alice,Charlie", candidates)
    assert result == [1, 3]
...
def test_resolve_names_dedup():
    ...
    result = svc._resolve_names("Alice,Alice", candidates)
    assert result == [1]


def test_resolve_names_none_text():
    """NONE 返回空列表"""
    from app.services.wakeup_service import WakeupService

    svc = WakeupService()
    assert svc._resolve_names("NONE", []) == []
    assert svc._resolve_names("", []) == []
    assert svc._resolve_names(None, []) == []
