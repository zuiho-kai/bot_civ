...
### 重要约束
- **⚠️ 禁止打包讨论**：不同维度的决策（如前端框架 vs 后端框架）必须拆开独立辩论，每个议题单独走辩论流程，单独记录结果。绝不能把多个独立问题捆绑成一个辩论。
- **⚠️ 辩论记录必须可见**：每轮辩论的详细内容（各方论点、质疑、回应）必须完整记录到 `docs/discussions.md`，不能只记录最终结果。`claude-progress.txt` 只记决策摘要。
